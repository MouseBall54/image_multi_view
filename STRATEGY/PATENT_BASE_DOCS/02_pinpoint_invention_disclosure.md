...
3. 따라서 권리화는 **좌표모델(refPoint/refScreen), 정/역변환 폐루프, 24뷰어 운영 워크플로, 동시 Pan 메커니즘의 결합 단위**로 설계해야 한다.
4. “완전 자동 정합”으로 과장하면 구현 사실과 불일치하므로 금지한다.
...
즉, 본 발명은 다음을 하나로 묶어 제공한다.
1. 창별 기준점 고정
2. 공통 화면 기준점 기반 동시 Pan
3. 정방향/역변환 폐루프 일관성
4. 최대 24 뷰어 확장 운영
5. 단일 이미지 다중 포인트 병렬 관찰

이 결합 구조는 선행기술의 자동 정렬 중심 흐름과 구분되며, compareX의 제품 차별성과 직결되는 직무발명으로 평가할 수 있다.

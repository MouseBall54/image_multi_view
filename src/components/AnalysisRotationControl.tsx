...
export function AnalysisRotationControl() {
  ...
  const isLevActive = levelingCapture.active && levelingCapture.mode === 'analysis' && (levelingCapture.targetKey == null);
  ...
  return (
    <div className="pinpoint-rotation-control">
      ...
      <button className={`viewer-control-btn${horizActive ? ' active' : ''}`} onClick={() => useStore.getState().startLeveling('analysis', null)} title="Level horizontally (pick 2 points)">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <button className={`viewer-control-btn${vertActive ? ' active' : ''}`} onClick={() => useStore.getState().startLeveling('analysis', null, 'vertical')} title="Level vertically (pick 2 points)">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">

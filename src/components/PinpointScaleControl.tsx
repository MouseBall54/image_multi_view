...
export function PinpointScaleControl({ folderKey, onResetRefPoint }: { folderKey: FolderKey, onResetRefPoint?: (key: FolderKey) => void }) {
  ...
  return (
    <div className="pinpoint-scale-control">
      ...
      <button className="zoom-out-button" onClick={() => adjustScale(1 / WHEEL_ZOOM_STEP)} title="Zoom Out">-</button>
      ...
      <button className="zoom-in-button" onClick={() => adjustScale(WHEEL_ZOOM_STEP)} title="Zoom In">+</button>
      <button className="reset-zoom-button" onClick={resetScale} title="Reset Zoom (center ref-point)" aria-label="Reset Zoom">
